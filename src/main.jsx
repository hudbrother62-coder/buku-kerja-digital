...
const previewData = () => ({
  ...
  events: [{ id: "event-preview", class_id: "class-7a", subject_id: null, title: "Rapat koordinasi guru", event_date: today(), start_time: "13:00", end_time: "14:00", event_type: "meeting", note: "Ruang guru", all_day: false }],
});
...
function writeSessionJson(key, value) {
  try { window.sessionStorage.setItem(key, JSON.stringify(value)); } catch { /* Cache is optional; a full cache must not break a successful save. */ }
}
...
async function fetchRemoteData(user) {
  ...
  const results = await Promise.all([
    ...
    supabase.from("teacher_events").select("*").eq("school_id", school.id).eq("user_id", user.id).order("event_date").order("start_time"),
  ]);
  const failed = results.find((result) => result.error);
  if (failed?.error) throw failed.error;
  const [years, classes, subjects, students, enrollments, attendance, journals, assessments, scores, schedules, events] = results.map((result) => result.data || []);
  ...
  data.grades = scores.map((score) => {
    ...
    return { id: score.id, assessment_id: score.assessment_id, student_id: score.student_id, student_name: student?.full_name || "", student_nisn: student?.nisn || "", class_name: classRow?.name || "", subject_name: subject?.name || "", assessment_title: assessment.title || "", assessment_category: assessment.category || "", assessment_date: assessment.assessment_date || "", point: score.point, max_point: assessment.max_point || 100, comment: score.comment || "" };
  });
  return data;
}
...
function Workspace({ auth, onLogout }) {
  ...
  useEffect(() => { refresh(!data.profile.setupComplete).catch(() => {}); }, [auth.user.id]);
  ...
  const handlers = {
    ...
    addGrade: async (draft) => {
      try { const result = await saveGradeRows([draft], data, auth, refresh, commit, false); if (!result.added) throw new Error("Nilai belum tersimpan. Periksa siswa, kelas, mata pelajaran, dan nilai."); notify("success", "Nilai berhasil disimpan."); return result; } catch (err) { notify("error", err.message || "Nilai belum tersimpan."); throw err; }
    },
    deleteRecord: async (kind, row) => {
      const tableByKind = { student: "students", class: "classes", subject: "subjects", journal: "teaching_journals", grade: "assessment_scores", schedule: "teacher_schedules", event: "teacher_events" };
      const collectionByKind = { student: "students", class: "classes", subject: "subjects", journal: "journals", grade: "grades", schedule: "schedules", event: "events" };
      const table = tableByKind[kind]; const collection = collectionByKind[kind];
      ...
      try {
        ...
        commit((current) => {
          const next = { ...current, [collection]: current[collection].filter((item) => item.id !== row.id) };
          if (kind === "student") {
            next.attendance = current.attendance.filter((item) => item.student_id !== row.id);
            next.grades = current.grades.filter((item) => item.student_id !== row.id);
          }
          if (kind === "class") {
            const studentIds = new Set(current.students.filter((item) => item.class_id === row.id).map((item) => item.id));
            next.students = current.students.filter((item) => item.class_id !== row.id);
            next.attendance = current.attendance.filter((item) => item.class_id !== row.id && !studentIds.has(item.student_id));
      ...
      } catch (err) { notify("error", err.message || "Data belum dapat dihapus."); throw err; }
    },
    ...
    saveEvent: async (draft, eventId = null) => {
      try {
        const row = { ...draft, id: eventId || id("event"), all_day: Boolean(draft.all_day) };
        if (auth.mode !== "preview") {
          const payload = { school_id: data.school.id, user_id: auth.user.id, class_id: draft.class_id || null, subject_id: draft.subject_id || null, title: draft.title, event_date: draft.event_date, start_time: draft.all_day ? null : draft.start_time, end_time: draft.all_day ? null : draft.end_time, event_type: draft.event_type, note: draft.note || null, all_day: Boolean(draft.all_day), updated_at: new Date().toISOString() };
  ...
  const page = active === "dashboard" ? <DashboardPage data={data} setActive={setActive} onPreferences={handlers.updatePreferences} />
    : active === "assistant" ? <AssistantPage data={data} />
      : active === "master" ? <MasterDataPage data={data} onSaveClass={handlers.saveClass} onSaveSubject={handlers.saveSubject} onSaveStudent={handlers.saveStudent} onDelete={handlers.deleteRecord} onImport={handlers.importMaster} />
        : active === "attendance" ? <AttendancePage data={data} onSave={handlers.saveAttendance} onImport={handlers.importAttendance} />
          : active === "journal" ? <JournalPage data={data} onAdd={handlers.addJournal} onDelete={handlers.deleteRecord} />
            : active === "grades" ? <GradesPage data={data} onMaster={() => setActive("master")} onImport={handlers.importGrades} onAdd={handlers.addGrade} onDelete={handlers.deleteRecord} />
              : active === "agenda" ? <AgendaPageV2 data={data} onSave={handlers.saveSchedule} onSaveEvent={handlers.saveEvent} onDelete={handlers.deleteRecord} onImport={handlers.importSchedules} />
  ...
  return <div className={dark ? "app dark" : "app"}>
    ...
    <main className="main"><header className="topbar"><button className="icon-button mobile-trigger" onClick={() => setMobileMenu(true)}><Menu size={19} /></button><div><span className="eyebrow">Buku Kerja Digital</span><h1>{NAV_ITEMS.find((item) => item.id === active)?.label || "Pengaturan"}</h1></div><div className="top-actions"><span className="connection-pill"><span></span>Database terhubung</span><div className="notification-wrap"><button className="theme-button notification-button" aria-label="Buka notifikasi" onClick={() => setNotificationsOpen((current) => !current)}><Bell size={17}/>{notifications.length > 0 && <b>{notifications.length}</b>}</button>{notificationsOpen && <div className="notification-panel"><div><strong>Notifikasi</strong><button className="icon-button" onClick={() => setNotificationsOpen(false)}><X size={15}/></button></div>{notifications.length ? notifications.map((item) => <button key={item.id} onClick={() => { setActive(item.id.startsWith("event-") ? "agenda" : item.id === "setup-students" ? "master" : "attendance"); setNotificationsOpen(false); }}><span></span><div><strong>{item.title}</strong><small>{item.detail}</small></div></button>) : <p>Semua pekerjaan penting sudah diperiksa.</p>}</div>}</div><button className="theme-button" onClick={() => setDark((current) => !current)}>{dark ? <Sun size={17} /> : <Moon size={17} />}</button><span className="top-avatar">{avatarName(data.profile.fullName)}</span></div></header><div className="content">{error && <div className="form-notice error page-notice">{error}</div>}{notice && <div className={"form-notice " + notice.type + " page-notice"}>{notice.message}</div>}{page}</div></main><nav className="mobile-nav">{[NAV_ITEMS[0], NAV_ITEMS[2], NAV_ITEMS[3], NAV_ITEMS[5], NAV_ITEMS[6]].map((item) => { const Icon = item.icon; return <button key={item.id} className={active === item.id ? "active" : ""} onClick={() => setActive(item.id)}><Icon size={18} /><span>{item.label.split(" ")[0]}</span></button>; })}</nav>
  </div>;
}

async function saveGradeRows(rows, data, auth, refresh, commit, skipExamples = true) {
  ...
  for (const raw of rows) {
    const row = normalizeRow(raw);
    if (skipExamples && isExampleRow(row)) { skipped += 1; continue; }
    ...
    if (!student || !className || !subjectName || !title || !text(row.point) || !Number.isFinite(point) || point < 0) { skipped += 1; continue; }
...
function ConfirmDelete({ title, description, busy, onCancel, onConfirm }) {
  return createPortal(<div className="confirm-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && onCancel()}>
    <section className="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="confirm-delete-title">
      <span className="danger-symbol"><Trash2 size={20}/></span>
      <div><p className="eyebrow">KONFIRMASI HAPUS</p><h2 id="confirm-delete-title">{title}</h2><p>{description}</p></div>
      <div className="confirm-actions"><button className="secondary-button" onClick={onCancel} disabled={busy}>Batal</button><button className="danger-button" onClick={onConfirm} disabled={busy}>{busy ? <LoaderCircle className="spin" size={16}/> : <Trash2 size={16}/>} {busy ? "Menghapus…" : "Hapus data"}</button></div>
    </section>
  </div>, document.body);
}
...
function MasterDataPage({ data, onSaveClass, onSaveSubject, onSaveStudent, onDelete, onImport }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      if (event.key === "Tab") {
        const items = [...dialog.querySelectorAll('button:not(:disabled), input:not(:disabled), select:not(:disabled), textarea:not(:disabled)')];
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try { await onDelete(deleteTarget.kind, deleteTarget.row); setDeleteTarget(null); }
  ...
  return <PageSection
    ...
    title="Master Data"
    action={<div className="section-actions"><ImportActions onImport={onImport} /><button className="primary-button" onClick={() => openCreate(data.classes.length ? "student" : "class")}><Plus size={16} /> Tambah master data</button></div>}
  >
    ...
    <div className="master-summary-grid"><article><span className="summary-icon purple"><Users size={18}/></span><div><strong>{data.students.length}</strong><small>Siswa aktif</small></div></article><article><span className="summary-icon blue"><GraduationCap size={18}/></span><div><strong>{data.classes.length}</strong><small>Kelas</small></div></article><article><span className="summary-icon amber"><BookOpen size={18}/></span><div><strong>{data.subjects.length}</strong><small>Mata pelajaran</small></div></article></div>
    <details className="master-catalog panel"><summary><div><p className="eyebrow">KELAS & MATA PELAJARAN</p><strong>Kelola data pendukung</strong></div><ChevronDown size={18}/></summary><div className="catalog-groups"><section><h3>Kelas</h3>{data.classes.map((row) => <div className="catalog-row" key={row.id}><span>{row.name}<small>{studentCountByClass.get(row.id) || 0} siswa</small></span><div><button className="icon-button" onClick={() => openEditClass(row)} aria-label={"Edit " + row.name}><Pencil size={14}/></button><button className="icon-button danger-icon" onClick={() => setDeleteTarget({kind:"class", row, title:`Hapus kelas ${row.name}?`, description:"Siswa di kelas ini beserta presensi, nilai, jurnal, dan agenda terkait akan ikut dihapus."})}><Trash2 size={14}/></button></div></div>)}</section><section><h3>Mata pelajaran</h3>{data.subjects.map((row) => <div className="catalog-row" key={row.id}><span>{row.name}<small>{row.code || "Tanpa kode"}</small></span><div><button className="icon-button" onClick={() => openEditSubject(row)} aria-label={"Edit " + row.name}><Pencil size={14}/></button><button className="icon-button danger-icon" onClick={() => setDeleteTarget({kind:"subject", row, title:`Hapus ${row.name}?`, description:"Mata pelajaran dihapus dan hubungan agenda terkait akan dibersihkan."})}><Trash2 size={14}/></button></div></div>)}</section></div></details>

    <div className="master-layout">
      <aside className="class-manager">
        <div className="panel-head"><div><p className="eyebrow">DAFTAR KELAS</p><h3>{data.classes.length} kelas</h3></div></div>
      ...
      <div className="master-content">
        ...
        <div className="table-card">{visible.length === 0 ? <EmptyState title="Belum ada siswa di kelas ini" desc="Tekan Tambah master data atau import template resmi."/> : visible.map((student) => <div className="data-row student-row" key={student.id}>
          ...
          <div className="row-actions"><button className="row-action" onClick={() => openEditStudent(student)}><Pencil size={15}/> Edit</button><button className="row-action danger" onClick={() => setDeleteTarget({kind:"student", row:student, title:`Hapus ${student.full_name}?`, description:"Data siswa, presensi, nilai, dan hubungan kelasnya akan dihapus secara permanen."})}><Trash2 size={15}/> Hapus</button></div>
        </div>)}</div>
      </div>
    </div>

    {modalOpen && createPortal(<div className="master-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) closeModal(); }}>
    ...
    {deleteTarget && <ConfirmDelete title={deleteTarget.title} description={deleteTarget.description} busy={deleting} onCancel={() => setDeleteTarget(null)} onConfirm={confirmDelete}/>} 
  </PageSection>;
}
...
function GradesPage({ data, onImport, onAdd, onMaster }) {
  const [showForm, setShowForm] = useState(true); const [busy, setBusy] = useState(false); const [message, setMessage] = useState(""); const lock = useRef(false); const [form, setForm] = useState({ student_id: "", class_id: data.classes[0]?.id || "", subject_id: data.subjects[0]?.id || "", assessment_title: "", assessment_category: "TUGAS", assessment_date: today(), point: "", max_point: "100", comment: "" });
  const update = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  ...
  const submit = async (event) => {
    ...
    setMessage("");
    if (!classRow || !selectedStudent || !subjectRow) return setMessage("Pilih kelas, siswa, dan mata pelajaran. Jika belum tersedia, lengkapi Master Data terlebih dahulu.");
    if (!form.assessment_title.trim() || !form.assessment_date) return setMessage("Isi nama tugas dan tanggal penilaian.");
    const point = Number(form.point), maximum = Number(form.max_point);
    if (form.point === "" || !Number.isFinite(point) || !Number.isFinite(maximum) || maximum <= 0 || point < 0 || point > maximum) return setMessage("Nilai harus antara 0 dan nilai maksimum yang lebih besar dari 0.");
    lock.current = true; setBusy(true);
    try {
      await onAdd({ ...form, student_name: selectedStudent.full_name, student_nisn: selectedStudent.nisn || "", class_name: classRow.name, subject_name: subjectRow.name });
      setMessage("Nilai berhasil disimpan. Pilih siswa berikutnya untuk tugas yang sama.");
      setForm(current => ({ ...current, student_id: "", point: "", comment: "" }));
    } catch (error) { setMessage(error.message || "Penyimpanan gagal. Isian tetap tersedia untuk dicoba kembali."); }
    finally { lock.current = false; setBusy(false); }
  };
  return <PageSection eyebrow="HASIL BELAJAR" title="Penilaian" action={<div className="section-actions"><ImportActions onImport={onImport}/><button className="primary-button" onClick={() => setShowForm((current) => !current)}><Plus size={16}/> Input nilai</button></div>}><div className="helper-banner"><ClipboardList size={18}/><span>Nama siswa otomatis dari Master Data. Catatan dapat dipakai untuk status belum tuntas, remedial, atau informasi lain.</span></div>{(!data.classes.length || !data.subjects.length || !data.students.length) && <div className="helper-banner"><span>Siapkan kelas, mata pelajaran, dan siswa sebelum mencatat nilai.</span><button className="secondary-button" onClick={onMaster}>Lengkapi Master Data</button></div>}{message && <div className="form-notice" role="status">{message}</div>}{showForm && <form className="inline-form grades-form" onSubmit={submit}><label className="field"><span>Kelas</span><select value={form.class_id} onChange={(event) => { update("class_id", event.target.value); update("student_id", ""); }}>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="field"><span>Nama siswa</span><select value={form.student_id} onChange={(event) => update("student_id", event.target.value)}><option value="">Pilih dari Master Data</option>{roster.map((item) => <option key={item.id} value={item.id}>{item.full_name}</option>)}</select></label><label className="field"><span>Mata pelajaran</span><select value={form.subject_id} onChange={(event) => update("subject_id", event.target.value)}>{data.subjects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><Field label="Nama tugas / asesmen" value={form.assessment_title} onChange={(value) => update("assessment_title", value)} placeholder="Contoh: Tugas 1"/><label className="field"><span>Kategori</span><select value={form.assessment_category} onChange={(event) => update("assessment_category", event.target.value)}><option>TUGAS</option><option>UH</option><option>PTS</option><option>PAS</option><option>PROYEK</option><option>LAINNYA</option></select></label><label className="field"><span>Tanggal</span><input type="date" value={form.assessment_date} onChange={(event) => update("assessment_date", event.target.value)}/></label><Field label="Nilai" value={form.point} onChange={(value) => update("point", value)} placeholder="0–100" type="number"/><Field label="Nilai maksimum" value={form.max_point} onChange={(value) => update("max_point", value)} placeholder="100" type="number"/><label className="field field-wide"><span>Catatan guru</span><textarea value={form.comment} onChange={(event) => update("comment", event.target.value)} rows="2" placeholder="Contoh: Belum tuntas, remedial hari Jumat"/></label><button type="submit" className="primary-button" disabled={busy}>{busy ? <LoaderCircle size={16} className="spin"/> : <Save size={15}/>} {busy ? "Menyimpan…" : "Simpan nilai"}</button></form>}<div className="grade-summary"><strong>{data.grades.length}</strong><span>baris nilai tersimpan</span><div className="grade-progress"><span style={{ width: data.students.length ? Math.min(100, Math.round((data.grades.length / Math.max(1, data.students.length)) * 100)) + "%" : "0%" }}></span></div></div><div className="table-card">{data.grades.length === 0 ? <EmptyState title="Belum ada nilai" desc="Input nilai dari daftar siswa atau import template resmi."/> : data.grades.map((grade) => <div className="data-row grade-row" key={grade.id}><span className="person-avatar blue">{avatarName(grade.student_name)}</span><div className="person-copy"><strong>{grade.student_name}</strong><small>{grade.class_name} · {grade.subject_name} · {grade.assessment_title}{grade.comment ? ` · ${grade.comment}` : ""}</small></div><strong className="score-value">{grade.point}</strong><span className="row-meta">/{grade.max_point || 100}</span></div>)}</div></PageSection>;
}
...
function AgendaPageV2({ data, onSave, onSaveEvent, onDelete, onImport }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null);
  const [busy, setBusy] = useState(false);
  const blankEvent = () => ({ title: "", event_date: selectedDate, class_id: data.classes[0]?.id || "", subject_id: "", start_time: "07:00", end_time: "08:00", event_type: "other", note: "", all_day: false });
  const blankSchedule = () => ({ day_of_week: 1, class_id: data.classes[0]?.id || "", subject_id: "", start_time: "07:00", end_time: "08:00", note: "" });
  ...
  const gridStart = new Date(cursor.getFullYear(), cursor.getMonth(), 1 - firstMondayOffset);
  ...
  const openCreate = (type, date = selectedDate) => { setMode(type); setEditing(null); setForm(type === "event" ? { ...blankEvent(), event_date: date } : blankSchedule()); setOpen(true); };
  const openEdit = (item) => { setMode(item.kind); setEditing(item.id); setForm(item.kind === "event" ? { title:item.title, event_date:item.event_date, class_id:item.class_id || "", subject_id:item.subject_id || "", start_time:text(item.start_time).slice(0,5) || "07:00", end_time:text(item.end_time).slice(0,5) || "08:00", event_type:item.event_type || "other", note:item.note || "", all_day:Boolean(item.all_day) } : { day_of_week:Number(item.day_of_week), class_id:item.class_id, subject_id:item.subject_id || "", start_time:text(item.start_time).slice(0,5), end_time:text(item.end_time).slice(0,5), note:item.note || "" }); setOpen(true); };
  const submit = async (event) => { event.preventDefault(); if (busy) return; setBusy(true); try { if (mode === "event") { if (!form.title.trim() || !form.event_date || (!form.all_day && (!form.start_time || !form.end_time || form.start_time >= form.end_time))) return; await onSaveEvent(form, editing); } else { if (!form.class_id || !form.start_time || !form.end_time || form.start_time >= form.end_time) return; await onSave(form, editing); } setOpen(false); } finally { setBusy(false); } };
  const changeMonth = (offset) => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + offset, 1));
  return <PageSection eyebrow="AGENDA GURU" title="Kalender kegiatan" action={<div className="section-actions"><ImportActions onImport={onImport}/><button className="primary-button" onClick={() => openCreate("event")}><Plus size={16}/> Tambah agenda</button></div>}>
    <div className="helper-banner"><CalendarDays size={18}/><span>Jadwal rutin muncul otomatis setiap minggu. Rapat, upacara, dan kegiatan mendadak dapat ditambahkan pada tanggal tertentu.</span></div>
    <div className="calendar-shell"><div className="calendar-toolbar"><button className="icon-button" onClick={() => changeMonth(-1)} aria-label="Bulan sebelumnya"><ChevronLeft size={19}/></button><h3>{monthTitle}</h3><button className="secondary-button compact" onClick={() => { setCursor(new Date(now.getFullYear(), now.getMonth(), 1)); setSelectedDate(today()); }}>Hari ini</button><button className="icon-button" onClick={() => changeMonth(1)} aria-label="Bulan berikutnya"><ChevronRight size={19}/></button></div><div className="calendar-weekdays">{DAY_NAMES.map((day) => <span key={day}>{day.slice(0,3)}</span>)}</div><div className="month-grid">{days.map((date) => { const iso = isoLocal(date), entries = entriesFor(date), outside = date.getMonth() !== cursor.getMonth(); return <button key={iso} className={`calendar-day ${outside ? "outside" : ""} ${selectedDate === iso ? "selected" : ""} ${iso === today() ? "today" : ""}`} onClick={() => setSelectedDate(iso)}><span>{date.getDate()}</span><div>{entries.slice(0,2).map((item) => <i className={item.kind} key={item.kind + item.id}>{item.title}</i>)}{entries.length > 2 && <small>+{entries.length - 2} lagi</small>}</div></button>; })}</div></div>
    <section className="selected-agenda"><div className="selected-agenda-head"><div><p className="eyebrow">AGENDA TERPILIH</p><h3>{new Intl.DateTimeFormat("id-ID", { weekday:"long", day:"numeric", month:"long", year:"numeric" }).format(new Date(selectedDate + "T00:00:00"))}</h3></div><div><button className="secondary-button" onClick={() => openCreate("schedule")}><Clock3 size={15}/> Jadwal rutin</button><button className="primary-button" onClick={() => openCreate("event", selectedDate)}><Plus size={15}/> Kegiatan satu kali</button></div></div><div className="agenda-list">{selectedEntries.length ? selectedEntries.map((item) => <article className={`agenda-entry ${item.kind}`} key={item.kind + item.id}><span className="agenda-time">{item.all_day ? "Seharian" : `${text(item.start_time).slice(0,5)}–${text(item.end_time).slice(0,5)}`}</span><div><strong>{item.title}</strong><small>{data.classes.find((row) => row.id === item.class_id)?.name || "Umum"}{item.note ? ` · ${item.note}` : ""}</small></div><span className="kind-pill">{item.kind === "schedule" ? "Rutin" : "Satu kali"}</span><button className="icon-button" onClick={() => openEdit(item)}><Pencil size={15}/></button><button className="icon-button danger-icon" onClick={() => setDeleteTarget(item)}><Trash2 size={15}/></button></article>) : <EmptyState title="Belum ada agenda" desc="Tambahkan kegiatan satu kali atau atur jadwal rutin untuk tanggal ini."/>}</div></section>
    {open && createPortal(<div className="master-modal-backdrop"><section className="master-modal agenda-modal" role="dialog" aria-modal="true"><div className="master-modal-head"><div><p className="eyebrow">{editing ? "EDIT AGENDA" : "AGENDA BARU"}</p><h2>{mode === "event" ? "Kegiatan satu kali" : "Jadwal rutin mingguan"}</h2></div><button className="icon-button" onClick={() => !busy && setOpen(false)}><X size={18}/></button></div><div className="master-modal-tabs"><button type="button" className={mode === "event" ? "active" : ""} onClick={() => !editing && openCreate("event")}><CalendarDays size={17}/>Satu kali</button><button type="button" className={mode === "schedule" ? "active" : ""} onClick={() => !editing && openCreate("schedule")}><Clock3 size={17}/>Rutin mingguan</button></div><form className="master-modal-form" onSubmit={submit}><div className="master-modal-body"><div className="master-form-grid">{mode === "event" && <><Field label="Nama kegiatan" value={form.title} onChange={(value) => update("title", value)} placeholder="Rapat, upacara, atau pengingat"/><label className="field"><span>Tanggal</span><input type="date" value={form.event_date} onChange={(event) => update("event_date", event.target.value)}/></label><label className="field"><span>Jenis kegiatan</span><select value={form.event_type} onChange={(event) => update("event_type", event.target.value)}><option value="meeting">Rapat</option><option value="ceremony">Upacara</option><option value="teaching">Pembelajaran</option><option value="reminder">Pengingat</option><option value="other">Lainnya</option></select></label><label className="check-field"><input type="checkbox" checked={form.all_day} onChange={(event) => update("all_day", event.target.checked)}/> Kegiatan seharian</label></>}{mode === "schedule" && <label className="field"><span>Hari</span><select value={form.day_of_week} onChange={(event) => update("day_of_week", Number(event.target.value))}>{DAY_NAMES.map((day,index) => <option value={index+1} key={day}>{day}</option>)}</select></label>}<label className="field"><span>Kelas</span><select value={form.class_id} onChange={(event) => update("class_id", event.target.value)}><option value="">Agenda umum</option>{data.classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="field"><span>Mata pelajaran</span><select value={form.subject_id} onChange={(event) => update("subject_id", event.target.value)}><option value="">Umum / wali kelas</option>{data.subjects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>{!(mode === "event" && form.all_day) && <><label className="field"><span>Mulai</span><input type="time" value={form.start_time} onChange={(event) => update("start_time", event.target.value)}/></label><label className="field"><span>Selesai</span><input type="time" value={form.end_time} onChange={(event) => update("end_time", event.target.value)}/></label></>}<label className="field field-wide"><span>Catatan</span><textarea rows="3" value={form.note} onChange={(event) => update("note", event.target.value)} placeholder="Lokasi, tujuan, atau hal yang perlu disiapkan"/></label></div></div><div className="master-modal-actions"><button type="button" className="secondary-button" onClick={() => setOpen(false)}>Batal</button><button className="primary-button" disabled={busy}>{busy ? <LoaderCircle className="spin" size={16}/> : <Save size={16}/>} {busy ? "Menyimpan…" : "Simpan agenda"}</button></div></form></section></div>, document.body)}
    {deleteTarget && <ConfirmDelete title={`Hapus ${deleteTarget.title || "agenda ini"}?`} description={deleteTarget.kind === "schedule" ? "Jadwal ini tidak akan muncul lagi pada minggu berikutnya." : "Kegiatan satu kali ini akan dihapus dari kalender."} busy={busy} onCancel={() => setDeleteTarget(null)} onConfirm={async () => { setBusy(true); try { await onDelete(deleteTarget.kind, deleteTarget); setDeleteTarget(null); } finally { setBusy(false); } }}/>} 
  </PageSection>;
}
